...
try {
    ...
    echo "<h2>Test Final Fix for Reject Request #69</h2>";
    ...
    if ($test_result) {
        ...
        echo "<h3>Process with Final Logic:</h3>";
        ...
        foreach ($attachment_strings as $index => $attachment_string) {
            if (!empty($attachment_string) && trim($attachment_string) !== '') {
                ...
                echo "<p>Filtered parts: " . count($filtered_parts) . " - [" . implode(', ', $filtered_parts) . "]</p>";
                ...
                if (count($filtered_parts) >= 4 && !empty($filtered_parts[0]) && !empty($filtered_parts[1])) {
                    ...
                    if (!in_array($original_name, $seen_original_names)) {
                        ...
                        echo "<p style='color: orange;'>⚠️ Skipped duplicate: {$original_name}</p>";
                    }
                } else {
                    echo "<p style='color: red;'>❌ ERROR: Insufficient parts (" . count($filtered_parts) . ")</p>";
                }
            } else {
                echo "<p style='color: gray;'>⚪ Empty string skipped</p>";
            }
        }
        ...
        if (count($original_names) === count($unique_original_names) && count($processed_attachments) === 2) {
            ...
            echo "<p>Expected 2 unique attachments, got " . count($processed_attachments) . "</p>";
            echo "</div>";
        }
